      ...
      if pos > -1:
        ...
        if pos > -1:
          ...
          if i == 0:
            ...
            try:
              #print 'date', trunk,
              date = fDates.transformBarDDMMYYYYDateIntoDatetime(trunk)
              #print 'date', date
              jogoReg['date'] = date
            except ValueError:
              continue
...
def produceTxtDataFileFromHtml(jogos, txtDataFilename):
  print 'Going to write', len(jogos), 'jogos'
...
def pickleJogosFromTxt(txtDataFilename, pickleFilename, nOfBits):
  ...
  print 'Dumping', pickleFilename, '--', len(jogos), 'jogos'
...
def unpickleJogosDataFile(pickleFilename, nDeDezenasSorteadas, nOfBits):
  ...
  for binDecReprJogo in binDecReprJogos:
    ...
    jogos.append(jogo)
  #print jogos
  print 'unpickled', len(jogos), 'jogos'
  return jogos  

pickleFilename = 'lf-jogos-sorteados-pickle.obj'
...
def checkTxtWithPickle(): #(txtDataFilename, pickleFilename):
  txtDataFilename = 'ms-jogos-sorteados.txt'
  pickleFilename =  'ms-jogos-sorteados.pickle.obj'
