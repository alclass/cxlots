...
FILT_STDDIF_C_LASTJOGO_MAIOR_Q_MAX = 237
  ...
  for i in range(nDoJogoToCompare, nDoJogoToCompare - goBackwardsNJogos, -1): # go backwards 3 jogosfs
...
def filtroHistogramaEvolutivo(jogo, nDoJogoToCompare=Sena.getNOfLastJogo()):
  '''
  This heuristic is relatively degradable ie as more jogosfs are added to history
...
FILT_REPEAT_COM_DEZENA_DO_JOGOREF = 56
  ...
  *Previous name of this method/function: filtroNDeIguaisJogoAntENDeJogosParaIguaisQuant()
  ...
  1 repeat   1 jogosfs ago
  1 repeat   2 jogosfs ago
  (...)
  1 repeat   5 jogosfs ago
  2 repeats   1 jogosfs ago
  2 repeats   2 jogosfs ago
  (...)
  3 repeats   10 jogosfs ago

  mecânica:
  1) positiveConditions:
     1.1) deve ter r 1-repeat em j jogosfs-ago (eg. 5 1-repeat entre os 7 últimos jogosfs.)
     1.2) deve ter r 2-repeat em j jogosfs-ago (eg. 3 2-repeat entre os 8 últimos jogosfs.)
  2) negativeCondition:
     2.1) não dever ter mais que r n-repeat em j jogosfs-ago (eg. não haver 3 3-repeat entre os 5 últimos jogosfs.)
...
class Filter2(object):
  ...
  def getSixtilStr(self):
    ...
    IMPORTANT NOTE: DO NOT CALL THIS METHOD FOR HISTORY JOGOS, instead CALL previous method which looks up a history table.
                    The sixtil string this method returns is a dynamic, on-the-fly one, only suitable for the filtering 'bet' jogosfs.
    '''                     
