...
1) a list called allowedFieldNamesInOriginalOrder
  This list just stores the fields in a Sena HTML
    table but with different names, for names were standardized here

2) a dict (hash map) called typeDict
  This dict keeps the pair fieldname and its expected python-type.
     Example: {nDoConcuso:int},
...
[1]
At the end, there is a string with all standardized names just to be used
  for comparison with allowedFieldNamesInOriginalOrder
If something differs, either order in which the fields must happpen,
  or fields quantity differs, execution will break.

[2]
A second comparison is to done again the HTML as it is updated
  when a new zip file is downloaded
...
if len(typeDict) != len(allowedFieldNamesInOriginalOrder):
  errmsg = "len(typeDict)=%d != len(allowedFieldNamesInOriginalOrder=%d)" % \
            (len(typeDict), len(allowedFieldNamesInOriginalOrder))
